...
describe('clamp', () => {
  test.failing('number is within the range', () => {
    expect(clamp(-7, -10, -5)).toBe(-7);
    expect(clamp(7, 5, 10)).toBe(7);
  });

  test.failing('number is outside the range', () => {
  ...
  test.failing('number is on the edge of the range', () => {
  ...
  test.failing('bounds are floats', () => {
  ...
  test.failing('number is boolean', () => {
  ...
  test.failing('bounds are booleans', () => {
  ...
  test.failing('bounds are strings', () => {
    expect(clamp(-5, '', 5)).toBe(0);
    expect(clamp(5, -5, '')).toBe(0);
